    ...
    if (request.status === 403 || request.status === 401 || request.status === 404) {
      dataInterface.selfProfile().done((data) => {
        if (data.message === 'user is not logged in') {
          let errorMessage = 'Please try reloading the page. If you need to contact support, note the following message: <pre>##server_message##</pre>';
          ...
          alertify.alert('You are not logged in', errorMessage);
        }
      });
    }
  });
