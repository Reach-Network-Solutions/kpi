...
import {isOnUsersRoute} from './usersUtils';
...
const usersStore = Reflux.createStore({
    ...
    init() {
      ...
      actions.users.searchUsers.started.listen(this.onSearchStarted);
      actions.users.searchUsers.completed.listen(this.onSearchCompleted);
      actions.users.searchUsers.failed.listen(this.onSearchFailed);
      actions.users.searchUsersMetadata.completed.listen(this.onSearchMetadataCompleted);
     ...
     */
    startupStore() {
      if (!this.isInitialised && isOnUsersRoute() && !this.data.isFetchingData) {
        this.fetchData(true);
      }
    },
    ...
    getSearchParams() {
      ...
      if (this.data.filterColumnId !== null) {
        const filterColumn = USERS_TABLE_COLUMNS[this.data.filterColumnId];
    ...
    onRouteChange(data) {
      if (!this.isInitialised && isOnUsersRoute() && !this.data.isFetchingData) {
        ...
        (
          ...
          this.previousPath.split('/')[1] !== 'users' ||
